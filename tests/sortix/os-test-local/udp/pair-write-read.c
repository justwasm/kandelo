/* Create two loopback UDP sockets connected to each other, write a datagram
   from the second socket to the first socket, and read it on the first. */
...
int main(void)
{
	...
	char x = 'x';
	if ( write(fd2, &x, sizeof(x)) < 0 )
		err(1, "write");
	usleep(50000);
	ssize_t amount = read(fd, &x, sizeof(x));
	if ( amount < 0 )
		err(1, "read");
	else if ( amount == 0 )
		puts("EOF");
